...
public class DesignTacoControllerTest {
	...
	public void setup() {
		...
		when(ingredientRepository.findById("FLTO").get()).thenReturn(
				new Ingredient("FLTO", "Flour Tortilla", Type.WRAP));
		when(ingredientRepository.findById("GRBF").get()).thenReturn(
				new Ingredient("GRBF", "Ground Beef", Type.PROTEIN));
		when(ingredientRepository.findById("CHED").get())
	...
	public void testShowDesignForm() throws Exception {
		mockMvc.perform(get("/design")).andExpect(status().isOk())
				...
				.andExpect(
						model().attribute("protein", ingredients.subList(2, 4)))
				.andExpect(
						model().attribute("veggies", ingredients.subList(4, 6)))
				.andExpect(
						model().attribute("cheese", ingredients.subList(6, 8)))
				.andExpect(
						model().attribute("sauce", ingredients.subList(8, 10)));
	}
